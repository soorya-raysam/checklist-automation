...
export default function CommandHistory() {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:5002/get-command-logs");
      ...
      if (json.error) {
        ...
        const distinct = [
          ...new Set(json.map((l) => l.command_name).filter(Boolean)),
        ];
  ...
  const downloadHistory = () => {
    window.open("http://localhost:5002/download-command-logs", "_blank");
  };
  ...
  const downloadExcel = (excelPath) => {
    ...
    window.open(
      `http://localhost:5002/download-excel/${encodeURIComponent(filename)}`,
      "_blank"
    );
  };
